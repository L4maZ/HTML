(function () {
  ...
  function apply(K) {
    ...
    var BOOKMAP = { TB_MSB: ['MSB', 0], BB_MSB: ['MSB', 1], TB_SBV: ['SBV', 0], BB_SBV: ['SBV', 1] };
  ...
  function ui() {
    ...
    if (!nav || document.querySelector('[data-nav="p6"]')) return false;
    ...
    btn.addEventListener('click', function (e) {
      ...
      e.preventDefault();
      var v = window.prompt('Nhập mật khẩu để mở tab Quản trị dữ liệu:');
      if (v === null) return;
      if (v !== PASS) { window.alert('Mật khẩu không đúng.'); return; }
      unlocked = true;
  ...
  window.RRTT_ADMIN = { apply: apply, log: function () { return LOG; }, source: function () { return SRC; } };
})();
