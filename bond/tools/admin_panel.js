  ...
  function apply(K) {
    ...
    if (Object.keys(K.ts).length) {
      ...
      Object.keys(K.ts).forEach(function (s) {
        ...
        var ix = TS_STALE.indexOf(target);
        if (ix >= 0) TS_STALE.splice(ix, 1);
        var src = K.ts[s], dst = {};
  ...
  function renderPanel() {
    ...
    if (TS_STALE && TS_STALE.length) {
      var NICE = {
        ...
        gdTPDN: 'Giao dịch TPDN', ytmRepo: 'Tương quan YTM vs repo',
        qlhsGov: 'QLHS GovBond', qlhsFi: 'QLHS FIBond & CD'
      };
      items.push(['no', 'Chuỗi biểu đồ không có trong file key — đang vẽ bằng số nhúng sẵn',
        TS_STALE.map(function (k) { return NICE[k] || k; }).join(' · ')]);
    }
